...
(JNIEnv *env, jobject obj, jint capType)
{
    try
    {
        WideVectorInfoClassInfo *classInfo = WideVectorInfoClassInfo::getClassInfo();
        ...
        if (!vecInfo)
            return;
        vecInfo->capType = (WideVectorLineCapType)capType;
    }
    catch (...)
    {
        __android_log_print(ANDROID_LOG_VERBOSE, "Maply", "Crash in WideVectorInfo::setCapTypeNative()");
    }
}
